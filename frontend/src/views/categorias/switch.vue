    ...
    <div class="flex items-center justify-center min-h-screen">
      <div class="max-w-7xl mx-auto mt-8 mb-8 bg-gray-900 p-8 rounded-lg shadow-lg w-full">
        <div class="flex justify-between items-center mb-2 ml-2">
          <h2 class="text-xl font-bold text-violet-700">NINTENDO SWITCH</h2>
        </div>
        ...
        <section>
          ...
          <!-- Si no hay juegos -->
          <div v-else-if="catSwitch.length === 0" class="text-center text-white">
          ...
          <div v-else class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-5 gap-6">
            <router-link v-for="producto in catSwitch" :key="producto.id" :to="`/producto/${producto.id}`" class="block">
...
onMounted(() => {
  axios.get('/productos/categorias/nintendo-switch')
    .then(res => {
      catSwitch.value = res.data
    })
    .catch(err => {
      console.error('Error al cargar productos de Nintendo Switch', err)
    })
    .finally(() => {
      loading.value = false
    })

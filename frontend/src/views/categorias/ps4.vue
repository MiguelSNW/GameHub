    ...
    <div class="flex items-center justify-center min-h-screen">
      <div class="max-w-7xl mx-auto mt-8 mb-8 bg-gray-900 p-8 rounded-lg shadow-lg w-full">
        <div class="flex justify-between items-center mb-2 ml-2">
          <h2 class="text-xl font-bold text-violet-700">PLAYSTATION 4</h2>
        </div>
        ...
        <section>
          ...
          <!-- Si no hay juegos -->
          <div v-else-if="catPs4.length === 0" class="text-center text-white">
...
onMounted(() => {
  axios.get('/productos/categorias/ps4')
    .then(res => {
      catPs4.value = res.data
    })
    .catch(err => {
      console.error('Error al cargar productos de PlayStation 4', err)
    })
    .finally(() => {
      loading.value = false
    })

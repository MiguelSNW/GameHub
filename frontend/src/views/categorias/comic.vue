<template>
  <div class="min-h-screen bg-cover bg-center" :style="{ backgroundImage: 'url(/images/wallpaper/COMIC.jpg)' }">
    <div class="flex items-center justify-center min-h-screen">
      <div class="max-w-7xl mx-auto mt-8 mb-8 bg-gray-900 p-8 rounded-lg shadow-lg w-full">
        <div class="flex justify-between items-center mb-2 ml-2">
          <h2 class="text-xl font-bold text-violet-700">Cómics</h2>
        </div>
        ...
        <section>
          ...
          <!-- Si no hay cómics -->
          <div v-else-if="catComic.length === 0" class="text-center text-white">
          ...
          <div v-else class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-5 gap-6">
            <router-link v-for="producto in catComic" :key="producto.id" :to="`/producto/${producto.id}`" class="block">
              <div class="bg-white p-4 rounded shadow transition-transform transform hover:-translate-y-1 hover:shadow-lg h-[400px] flex flex-col justify-between">
                <img :src="`/${producto.imagen}`" alt="Portada del cómic" class="h-[250px] w-full object-contain mb-4 rounded" />
                <div>
...
onMounted(() => {
  axios.get('/productos/categorias/comic')
    .then(res => {
      catComic.value = res.data
    })
    .catch(err => {
      console.error('Error al cargar productos de Cómics', err)
    })
    .finally(() => {
      loading.value = false
    })
